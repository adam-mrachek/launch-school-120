...
# The `serial_number` method was a private method which cannot have an explicit caller ('other' in this case) even inside the class.
# If we move the `serial_number` method to protected, we can access it with an explicit caller from within the class while keeping it inaccessible outside the class.